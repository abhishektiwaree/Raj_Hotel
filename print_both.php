...
if(isset($_GET['id'])){
	...
	if(isset($_GET['cid'])){
		$allot=$details['allotment_date'];
		$allot=date('Y-m-d',strtotime($allot));
		$exitdate=$details['exit_date'];
		$exitdate=date('Y-m-d',strtotime($exitdate));
		$custid=$_GET['cid'];
		$sql11="SELECT *  FROM `customer_transactions` WHERE cust_id='".$custid."' and DATE(timestamp)>='".$allot."' and DATE(timestamp)<='".$exitdate."' and type='sale_restaurant' and remarks='credit'";
		$foodres=execute_query($sql11);
		//echo $sql11;
	}
	//echo $days;
...
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
	...
	<div id="wrapper" style="page-break-after:avoid; margin-left:25px;margin-top:25px;"><br>
		...
		<?php}?>
			<tr>
				<td colspan=""><?php echo date('d-m-y',strtotime($food['timestamp'])); ?></td>
				<td>-</td>
				<td>Fooding charge</td>
				<td><?php echo $food['amount'];$tot+=$food['amount'];?>Rs.</td>
			</tr>
			<?php 
...
<?php} 
	...
	<div id="wrapper" style="page-break-after:avoid; margin-left:25px;margin-top:15px;"><br>
		<table width="100%" border="0" style="border-bottom: 1px solid;">
			<tr>
				<th colspan="2"><h1>HOTEL GONARD</h1></th>
			</tr>
			...
			<tr>
				<th colspan="2">Civil Lines, Gonda-271001</th>
			</tr>
			<tr>
				<td align="left">GSTIN : 09AACCP0999C1ZB</td>
			</tr>
			<tr>
				<td align="left">PAN No. : AACCP0999C</td>
				
			</tr>
			<tr>
				<td align="left">S.Tax No : 09AACCP0999C1ZB</td>
		...
		<table width="100%" class="td-center">
			...
			<tr>
				<th width="20%">Date</th>
				<th width="20%">Recpt/kot No.</th>
				<th width="10%">Room No.</th>
				<th width="30%">Charge Of</th>
			...
			?>
			<tr>
				<td><?php echo date("d-m-Y",strtotime($start)); $to=strtotime($start); $date_to = strtotime("+1 day", $to);?><br> To <br><?php echo date("d-m-Y",$date_to); ?></td>
				<?php
					echo '<td>'.$room_details['floor_name'].','.$room_details['room_type'].'</td>';
					$discount +=$details['discount_value'];
					$base_rent = $details['room_rent'];
				...
				<td>Room charges @ <?php echo $details['room_rent'];?>/-</td>
				<td><?php echo $details['room_rent']; ?>Rs.</td>
			</tr>
			<?php } ?>
			<?php
			$food_tot=0;
				while($food=mysqli_fetch_array($foodres)){
			?>
					<tr>
						<td colspan=""><?php echo date('d-m-y',strtotime($food['timestamp'])); ?></td>
						<td>GH/2019/<?php echo $food['invoice_no'];?></td>
						<td>-</td>
						<td>Fooding charges</td>
						<td><?php echo $food['amount'];$food_tot+=$food['amount']; //$tot+=$food['amount'];?>Rs.</td>
					</tr>
				<?php
				}
			?>
		...
		<tr>
			...
			<td> <?php echo $tot+$food_tot; ?>Rs.  </td>
			
		</tr>
		<tr>
			<td colspan="<?php echo $colspan-1; ?>" >SGST 6% on amount [<?php echo $tot; ?>]</td>
			<td> <?php if( $details['room_rent']>1000){ $sgst= $tot*6/100; echo $sgst;} else{$sgst=0;echo"0";} ?>Rs.</td>
		</tr>
		<tr>
			<td colspan="<?php echo $colspan-1; ?>" >CGST 6% on amount [<?php echo $tot;  ?>]</td>
			<td><?php if( $details['room_rent']>1000){ $cgst= $tot*7/100; echo $cgst;}else{$cgst=0;echo"0";} $tot=$tot+$food_tot;?>Rs.</td>
		</tr>
		<tr>
			<td colspan="<?php echo $colspan+1; ?>" style="border-top:1px solid;">
		...
		<tr>
			...
			<td><?php $withgst= $tot+$cgst+$sgst; echo round($withgst,2); ?>Rs.</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<tr >
				...
				<th colspan="<?php echo $colspan; ?>" align="right" >Mode of Payment : </th>
				<th><?php echo $cust_transact['mop']; ?></th>
			</tr>
			<tr>
				<td colspan="<?php echo $colspan+1; ?>" style="border-top:1px solid;"></td>
			</tr>
			<tr>
				<td colspan="<?php echo $colspan-4; ?>" style="text-align:left"><span style="text-transform: capitalize;">Rupees(inwords) : <?php echo int_to_words(round($tot,0)); ?> </span>
				</td>
			</tr>
			<tr>
				<td  colspan="<?php echo $colspan-5; ?>"  style="text-align:left">Signature of gust : <br> E.& O.E.</td>
				<td  colspan="<?php echo $colspan+2; ?>" style="text-align:right">For : HOTEL GONARD<br /><br /></td>
			</tr>
			<tr>
				<td colspan="<?php echo $colspan-5; ?>" align="center">Cashier</td>
				<td colspan="<?php echo $colspan; ?>"align="right">Receptionist</td>
			</tr>
			<tr>
