...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']==''){
    	$sql='INSERT INTO `restaurant_proforma_invoice`(`guest_name`, `mob_no`, `company_name`, `address`, `gstin`, `pin_code`, `cindt`, `remarks`, `quantity`,`days`, `amount`, `sgst`, `cgst`, `totel`, `created_by`, `creation_time`) VALUES ("'.$_POST['cust_name1'].'", "'.$_POST['mobile'].'", "'.$_POST['company_name'].'", "'.$_POST['address'].'", "'.$_POST['id_2'].'", "'.$_POST['zipcode'].'", "'.$_POST['cindt'].'", "'.$_POST['remarks'].'", "'.$_POST['quantity'].'", "'.$_POST['days'].'", "'.$_POST['amount'].'", "'.$_POST['sgst'].'", "'.$_POST['cgst'].'", "'.$_POST['grand_total'].'", "'.$_SESSION['username'].'","'.date("d-m-Y H:i:s").'")';
    	$res=execute_query($sql);
    	$inserted_id=mysqli_insert_id($db);
	}
	else{
	    $sql = '`guest_name` = "'.$_POST['cust_name1'].'", 
        `mob_no` = "'.$_POST['mobile'].'", 
        ...
        `sgst` = "'.$_POST['sgst'].'",  
        `cgst` = "'.$_POST['cgst'].'",
        `totel` =  "'.$_POST['grand_total'].'", 
        `created_by` = "'.$_SESSION['username'].'",
        `creation_time` = "'.date("d-m-Y H:i:s").'"
        where sno="'.$_POST['edit_sno'].'"';
        $res=execute_query($sql);
        $inserted_id = $_POST['edit_sno'];
        
        $sql = 'delete from restaurant_proforma_transition where proforma_invoice_sno="'.$_POST['edit_sno'].'"';
        execute_query($sql);
	    
	}
	for($i=1;$i<$_POST['insert_id'];$i++){
	    if($_POST['quantity_'.$i]!='' && $_POST['quantity_'.$i]!='0'){
    		$insql='INSERT INTO `restaurant_proforma_transition`(`proforma_invoice_sno`, `particulars`, `rate`,  `quantity`, `days`, `amount`, `sgst`, `cgst`, `total`, `creation_time`) VALUES ("'.$inserted_id.'", "'.$_POST['particular_'.$i].'","'.$_POST['rate_'.$i].'","'.$_POST['quantity_'.$i].'","'.$_POST['days_'.$i].'","'.$_POST['amount_'.$i].'","'.$_POST['sgst_'.$i].'","'.$_POST['cgst_'.$i].'","'.$_POST['grand_total_'.$i].'","'.date("d-m-Y H:i:s").'")';
	...
	if($msg==""){
	    $msg = '<li class="error">Data Inserted &nbsp; <a href="restaurant_print_proforma.php?id='.$inserted_id.'" target="_blank">Print</a></li>';
	}
...
if(isset($_GET['e_id'])){
 	$sql = 'SELECT * FROM `restaurant_proforma_invoice` WHERE `sno`="'.$_GET['e_id'].'"';
 	//echo $sql;
...
<div id="container">
        <h2>Restaurant Proforma Entry</h2>	
		...
		<form action="<?php echo $_SERVER['PHP_SELF']?>" class="wufoo leftLabel page1"  name="addnewdesignation" enctype="multipart/form-data" method="post" onSubmit="" >
			<table>
				...
				<!---<tr>
					...
					<td>
						<input type="hidden" id="coutdt" name="coutdt" value="<?php //if(isset($_GET['e_id'])){echo $row_customer_edit['coutdt'];} ?>" class="field text medium" maxlength="255" tabindex="<?php //echo $tab++;?>" type="text" />
					</td>
				</tr>--->
				<tr>
...
?>
			...
			<?php 
			if(isset($_GET['e_id'])){
			    $sql = 'select * from restaurant_proforma_transition where proforma_invoice_sno="'.$row_customer_edit['sno'].'"';
			...
			<tr style="background: <?php echo $col; ?>;text-align: center;" width="100%">
				...
				<!--	<input type="hidden" name="days_<?php //echo $i; ?>" value="1"  id="days_<?php// echo $i; ?>" class="field text medium" tabindex="<?php //echo $tab++;?>" style="width:100%;" onblur="insert_row();" >
				--->
				<td>
			...
			<tr style="background: <?php echo $col; ?>;text-align: center;" width="100%">
				...
				<td>
					...
					<input type="hidden" name="days_<?php echo $i; ?>" id="days_<?php echo $i; ?>" class="field text medium" tabindex="<?php echo $tab++;?>" style="width:100%;" onblur="insert_row();" value="<?php echo $row_particular_edit['quantity']; ?>" >
	...
	function insert_row(){
		...
		for (var k = 1; k <= i; k++) {
			...
			if(particular == ''){
				...
				// 	cgst=0;
				// }
				var sgst = (amount * 2.5)/100;
				var cgst = (amount * 2.5)/100;
