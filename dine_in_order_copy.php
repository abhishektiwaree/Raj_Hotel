...
if(isset($_GET['edit_id'])){
	$sql="DELETE FROM `kitchen_ticket_temp_2`";
	execute_query($sql);
	$sql_invoice='select * from `invoice_sale_restaurant_2` where `invoice_sale_id`="'.$_GET['edit_id'].'"';
	...
	$sql_stock='select * from `stock_sale_restaurant_2` where `invoice_no`="'.$_GET['edit_id'].'"';
	$result_stock=execute_query($sql_stock);
	echo '<script>alert('.$sql_stock.');</script>';
	while($row_stock=mysqli_fetch_array($result_stock)) {
		...
		$row_stock_available=mysqli_fetch_array($result_stock_available);
	 	$sql_ktt= 'INSERT INTO `kitchen_ticket_temp_2`(`unit`,`item_id`, `item_name`, `item_price`, `table_id`,`time_stamp`,`kot_type`,`kot_no`) VALUES("'.$row_stock['qty'].'","'.$row_stock['part_id'].'","'.$row_stock_available['description'].'","'.$row_stock_available['mrp'].'","'.$row_invoice['storeid'].'","'.date("Y-m-d H:i:sa").'","'.$kot_type.'","'.$row_invoice['kot_no'].'")';
...
if(isset($_POST['nocharge_bill'])){
	...
	$flag=0;
	$grand_total=0;
	$sql = 'select * from invoice_sale_restaurant_2 order by abs(nck_no) desc limit 1';
		//echo $sql;
	...
	for($i=1;$i<=$end;$i++){
        ...
        $total=$unit*$price;
        $grand_total+=$total;
		$sql = "INSERT INTO `stock_sale_restaurant_2` ( `supplier_id`, `part_id`, `basicprice`, `discount`, `discount_value`, `vat`, `vat_value`, `excise`, `excise_value`, `taxable_amount`, `effective_price`, `qty`, `part_dateofpurchase`, `amount`, `unit`, `admin_remarks`,`nck_no`) 
	...
	if(isset($_POST['done'])){
		...
		$flag=0;
		$sql = 'select * from invoice_sale_restaurant_2  order by abs(kot_no) desc limit 1';
		$kot_res = execute_query($sql);
		$sql="SELECT * FROM `kitchen_ticket_temp_2` order by abs(kot_no) desc limit 1";
		...
		for($i=1;$i<=$end;$i++){
	        ...
	        $sql="INSERT INTO `kitchen_ticket_temp_2`(`unit`,`item_id`, `item_name`, `item_price`, `table_id`,`time_stamp`,`kot_type`,`kot_no`) VALUES('$unit','$curr_sno','$item','$price','$table','$timestamp','$kot_type','$kotno')";
		...
		header("location:bill_invoice_copy.php?table_id=$table&edit_id=$editid");
		...
		<form action="" class="wufoo leftLabel page1"  name="addnewdesignation" enctype="multipart/form-data" method="post" onSubmit="">
			...
			<?php
				...
				while($row=mysqli_fetch_array($res)){
                    ...
                    if($row['booked_status'] ==1){
				        echo '<button type="button" onclick="window.open(\'dine_in_order_copy.php?table_id='.$id.'\', \'_self\');" id="table_'.$id.'" style="background-color:red;">'.$row['table_number'].'</button></a>';
				    }
                    else{
                         echo '<button type="button"  onclick="window.open(\'dine_in_order_copy.php?table_id='.$id.'\', \'_self\');" id="table_'.$id.'">'.$row['table_number'].'</button></a>';
                    }
                }
			...
			<?php
				...
				while($row=mysqli_fetch_array($res)){
					$id=$row['sno'];
                    if($row['booked_status'] == 1){
				        echo '<button type="button" onclick="window.open(\'dine_in_order_copy.php?room_id='.$id.'\', \'_self\');" id="table_'.$id.'" style="background-color:red;">'.$row['room_name'].'</button></a>';
				    }
                    else{
                         echo '<button type="button"  onclick="window.open(\'dine_in_order_copy.php?room_id='.$id.'\', \'_self\');" id="table_'.$id.'">'.$row['room_name'].'</button></a>';
                    }
                }
...
<?php
	...
	case 2:{
		...
		$totPrice=0;
		$sql="SELECT * FROM `kitchen_ticket_temp_2` WHERE table_id='$tab_id'";
		$res=execute_query($sql);
		...
		function reduce_kot(id){
			...
			$.ajax({
				async: false,
				url: "scripts/ajax.php?id=red_kot_copy&term="+id,
				dataType: "json"
			})
			.done(function(data) {
				data = data[0];
		...
		<div id="container">
			<?php echo '<ul><h4>'.$msg.'</h4></ul>'; ?>
			<form action="dine_in_order_copy.php" class="wufoo leftLabel page1"  name="purchase_report" id="purchase_report" enctype="multipart/form-data" method="post" onSubmit="" >
				...
				<div id="left_container">
					<div id="left_div">
						...
						<input type="hidden" name="edit_id" value="<?php if(isset($_GET['edit_id'])){ echo $_GET['edit_id']; } ?>">
					</div>
					<div id="total_show">
						...
						<div id="total_bill">
							<?php if(isset($_GET['edit_id'])){
						?><a href="bill_invoice_copy.php?table_id=<?php echo $tableid; if(isset($_GET['edit_id'])){echo '&edit_id='.$_GET['edit_id'];}?>"><?php } ?> <button type="button" id="total_cal"><h3>Total : Rs.<span id="show_total"><?php echo $total; ?></span></h3></button></a>
						</div>
					...
					<div id="div_button">
						...
						if (isset($_GET['edit_id'])) {
							...
							if($pos !== false){
								...
								echo '<a href="change_table_bill_edit.php?roomid='.$room_details['sno'].'&e_id='.$_GET['edit_id'].'"><button type="button" name="Table" >Change Room: '.$room_details['room_name'].'</button></a>';
							}
							else{
								...
								echo '<a href="change_table_bill_edit.php?tableid='.$table_name['sno'].'&e_id='.$_GET['edit_id'].'"><button type="button" name="Table" >Change Table: '.$table_name['table_number'].'</button></a>';
							}
						}
						elseif(isset($_GET['table_id'])){
