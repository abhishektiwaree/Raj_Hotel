...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		if($_POST['cust_sno']=='' OR isset($_POST['new_user'])){
			$sql= 'INSERT INTO customer (company_name, cust_name, mobile, id_1, id_2, id_3, id_type, address, created_by, created_on , state) VALUES ("'.$_POST['company_name'].'", "'.$_POST['cust_name1'].'", "'.$_POST['mobile'].'", "'.$_POST['id_1'].'", "'.$_POST['id_2'].'", "'.$_POST['id_3'].'", "'.$_POST['id_type'].'", "'.$_POST['address'].'", "'.$_SESSION['username'].'" ,"'.date('Y-m-d H:i:s').'" , "UTTAR PRADESH")';
			$result = execute_query($sql);
		...
		foreach($_POST['room_id'] as $k => $v){
			...
			if($_POST['allot2_sno'] !=''){
				...
				$sql = 'INSERT INTO allotment_2 (guest_name, cust_id, room_id , room_rent, discount, discount_value, other_discount, original_room_rent, financial_year, invoice_no, registration_no, invoice_type, tax_rate, allotment_date, created_by , created_on, remarks, occupancy, other_charges, edited_by, edited_on,plans,taxable_amount,allotment_id,edit_status) VALUES ("'.$_POST['cust_name1'].'","'.$_POST['cust_sno'].'", "'.$v.'" , "'.$grand_tot.'", "'.$_POST['discount_'.$v].'", "'.$_POST['discount_value_'.$v].'", "'.$dis.'", "'.$_POST['room_'.$v].'", "'.$year.'", "'.$_POST['invoice_no'].'", "'.$_POST['registration_no'].'", "'.$inv_type.'", "'.$tax_rate.'",  "'.$_POST['allotment_date'].'", "'.$_SESSION['username'].'" ,CURRENT_TIMESTAMP , "'.$_POST['remarks'].'","'.$_POST['occupancy_'.$v].'", "'.$_POST['extrabed_'.$v].'", '.$edited_by.', '.$edition_time.',"'.$_POST['plan'].'","'.$taxable_amt.'","'.$_POST['allot_sno'].'","1")';
				$result = execute_query($sql);
			}
			else if($_POST['id'] !=''){
				...
				$sql = 'INSERT INTO allotment_2 (guest_name, cust_id, room_id , room_rent, discount, discount_value, original_room_rent, financial_year, invoice_no, registration_no, invoice_type, tax_rate, allotment_date, exit_date, created_by , created_on, remarks, occupancy, other_charges, edited_by, edited_on,plans,taxable_amount,allotment_id,edit_status) VALUES ("'.$_POST['cust_name1'].'","'.$_POST['cust_sno'].'", "'.$v.'" , "'.$grand_tot.'", "'.$_POST['discount_'.$v].'", "'.$_POST['discount_value_'.$v].'", "'.$_POST['room_'.$v].'", "'.$year.'", "'.$_POST['invoice_no'].'", "'.$_POST['registration_no'].'", "'.$inv_type.'", "'.$tax_rate.'",  "'.$_POST['allotment_date'].'", '.$exit_date.', "'.$_SESSION['username'].'" ,CURRENT_TIMESTAMP , "'.$_POST['remarks'].'","'.$_POST['occupancy_'.$v].'", "'.$_POST['extrabed_'.$v].'", '.$edited_by.', '.$edition_time.',"'.$_POST['plan'].'","'.$taxable_amt.'","'.$_POST['allot_sno'].'","1")';
				$result = execute_query($sql);
				...
				$sql = 'INSERT INTO allotment_2 (guest_name, cust_id, room_id , room_rent, discount, discount_value, original_room_rent, financial_year, invoice_no, registration_no, invoice_type, tax_rate, allotment_date, exit_date, created_by , created_on, remarks, occupancy, other_charges, edited_by, edited_on,plans,taxable_amount,allotment_id) VALUES ("'.$_POST['cust_name1'].'","'.$_POST['cust_sno'].'", "'.$v.'" , "'.$grand_tot.'", "'.$_POST['discount_'.$v].'", "'.$_POST['discount_value_'.$v].'", "'.$_POST['room_'.$v].'", "'.$year.'", "'.$_POST['invoice_no'].'", "'.$_POST['registration_no'].'", "'.$inv_type.'", "'.$tax_rate.'",  "'.$_POST['allotment_date'].'", '.$exit_date.', "'.$_SESSION['username'].'" ,CURRENT_TIMESTAMP , "'.$_POST['remarks'].'","'.$_POST['occupancy_'.$v].'", "'.$_POST['extrabed_'.$v].'", '.$edited_by.', '.$edition_time.',"'.$_POST['plan'].'","'.$taxable_amt.'","'.$alot_id.'")';
				$result = execute_query($sql);
			}
		...
		<form action="allotment2.php" class="wufoo leftLabel page1"  name="addnewdesignation" enctype="multipart/form-data" method="post" onSubmit="" >
			...
			<table>
				...
				<td>Registation Number</td>
					<td><input id="registration_no" name="registration_no" value="<?php if(isset($_GET['check_in'])){echo $old_data['registration_no'];}?>" class="field text medium" maxlength="255" tabindex="<?php echo $tab++;?>" type="text" /></td>
				</tr>
				<tr>
					<td>Company Name</td>
						...
						<table width="100%">
							<tbody>
								...
								<tr>
									...
									<td><input type="text" name="net_room_rent_<?php echo $room_details['sno']; ?>" id="net_room_rent_<?php echo $room_details['sno']; ?>" value="<?php echo $old_data['room_rent']; ?>" onblur="calculate()" class="small"></td>
								</tr>
							</tbody>
						</table>
