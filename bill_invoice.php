...
if(isset($_POST['total_amount_hidden'])){
	...
	if($_POST['edit_pass'] != '') {
		...
		if(isset($_GET['room_id'])){
			...
			if(mysqli_num_rows($invoice_result)!=0){
				$invoice_no = mysqli_fetch_array($invoice_result);
				$_POST['invoice_no'] = str_replace("TB","",$invoice_no['invoice_no']);
				$_POST['invoice_no'] = $_POST['invoice_no']+1;
				$_POST['invoice_no'] = 'TB'.$_POST['invoice_no'];
			}
			else{
				$_POST['invoice_no'] =  "TB1";
			}
		}
...
function calculate(){
	...
	if(service_charge){
		if(service_charge>0){
		    ...
		    $("#service_charge_tax_amount").val('0');
    		$("#service_charge_tax_rate").val('0');
    		$("#service_charge_tax").html('0');
    		$("#service_charge_amt").html('0')
    		$("#service_charge_rate_amount").val('0');
		    
		}
		//alert(service_charge+' >> '+service_1charge_value);
	}
	else{
	    $("#service_charge_tax_amount").val('0');
		$("#service_charge_tax_rate").val('0');
	...
	<form action="bill_invoice.php?<?php echo $param;?>" id="purchase_report" class="wufoo leftLabel page1"  name="addnewdesignation" enctype="multipart/form-data" method="post" onsubmit="return confirm('Are you sure ?');">
		<div id="bill">
			<table style="background-color:#DDDDDD;border:1px solid black;">
				...
				if(isset($_GET['table_id'])){
					...
					while($row=mysqli_fetch_array($res)){
						$tot_tax_rate = (float)$row['vat']+(float)$row['excise'];
						//$base_price = round($row['item_price']/(1+($tot_tax_rate/100))/$row['qty'],2);
						$base_price=$row['item_price'];
						$taxable = $base_price*$row['qty'];
						$taxable = floatval($taxable); 
						$vat = floatval($row['vat']); 
						$cgst_amt = ($taxable * $vat) / 100;
						$taxable = floatval($taxable); // Convert $taxable to float
						$excise = floatval($row['excise']); // Convert $row['excise'] to float
						$sgst_amt = ($taxable * $excise) / 100;

						$item_price = floatval($row['item_price']); // Convert item_price to float
   						$vat = floatval($row['vat']); // Convert vat to float

   						$e_price = $item_price + (($item_price * $vat) / 100) + (($item_price * $vat) / 100);

						$e_price = floatval($e_price); // Convert $e_price to float
                        $qty = floatval($row['qty']); // Convert $row['qty'] to float
                        $total = $e_price * $qty;
		...
		<div id="div_total">
			<table width="100%">
				...
				<tr>
					...
					<td class="right" id="service_charge_row"><input type="text" name="service_charge_rate" id="service_charge_rate" placeholder="Rate" class="small" value="<?php if(isset($_GET['edit_id'])) { echo $row_discount_waitor['service_charge_rate']; }else{echo '5';}  ?>" style="float:left; width: 50px;" onBlur="calculate();"></td>
				</tr>
				<tr>
		...
		<div id="div_button">
			...
			<table>
				...
				<tr>
					<td>
						<input style="width: 200px;" type="text" name="company_name" id="company_name" placeholder="Company Name" class="medium" value="<?php if(isset($_GET['edit_id'])) { echo $row_customer['company_name']; }elseif($fill == "1"){echo $row_cust_room['company_name'];}  ?>">
					</td>
					<td>&nbsp;</td>
				</tr>
				
				<tr>
					<td><input style="width: 200px;" type="text" name="customer_name" id="customer_name" placeholder="Guest Name" class="medium" value="<?php if(isset($_GET['edit_id'])) { echo $row_customer['cust_name']; }elseif($fill == "1"){echo $row_cust_room['cust_name'];}  ?>"><input type="hidden" name="customer_sno" id="customer_sno" value="<?php if(isset($_GET['edit_id'])) { echo $row_customer['sno']; }elseif($fill == "1"){echo $row_cust_room['sno'];}  ?>"></td>
					<input style="width: 200px;" type="hidden" name="room_allot_id" id="room_allot_id" value="<?php if($fill == "1"){echo $row_inv['sno'];} ?>">
					<input style="width: 200px;" type="hidden" name="c" id="c" value="<?php  if(isset($_GET['edit_id'])) { echo $row_customer1['allotment_id']; }  ?>">
					<td><input style="width: 200px;" type="text" name="customer_mobile" id="customer_mobile" placeholder="Mobile Number" class="medium" value="<?php if(isset($_GET['edit_id'])) { echo $row_customer['mobile']; }elseif($fill == "1"){echo $row_cust_room['mobile'];}  ?>"></td>
				</tr>
				<tr>
					<td><input style="width: 200px;" type="text" name="customer_address" id="customer_address" placeholder="Address" class="medium" value="<?php if(isset($_GET['edit_id'])) { echo $row_customer['address']; }elseif($fill == "1"){echo $row_cust_room['address'];}  ?>"></td>
					<td><input style="width: 200px;" type="text" name="customer_gstin" id="customer_gstin" placeholder="GSTIN" class="medium" value="<?php if(isset($_GET['edit_id'])) { echo $row_customer['id_2']; }elseif($fill == "1"){echo $row_cust_room['id_2'];}  ?>"></td>
				</tr>
				<tr>
					<td> <input style="width: 200px;" type="text" name="waitor_name" id="waitor_name" placeholder="Waitor Name" class="medium" value="<?php if(isset($_GET['edit_id'])) { echo $row_discount_waitor['waitor_name']; } ?>"><input type="hidden" name="waitor_id" id="waitor_id"></td>
					<td><input style="width: 200px;" type="checkbox" name="tableprint" value="" checked>Print Table </td>
				</tr>
			</table>
