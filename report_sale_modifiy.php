...
if(isset($_GET['del'])){
	$sql = 'delete from invoice_sale_restaurant_2 where sno='.$_GET['del'];
	execute_query($sql);
	$sql = 'delete from stock_sale_restaurant_2 where invoice_no='.$_GET['del'];
	execute_query($sql);
	$sql = 'delete from customer_transactions_2 where number='.$_GET['del'].' and type="sale_restaurant"';
	execute_query($sql);
	$sql='delete from barcode_new where number='.$_GET['del'].' and type="sale"';
	execute_query($sql);
}
if(isset($_SESSION['sale_date_from'])){
	$sql = 'select invoice_sale_restaurant_2.sno, cust_name,company_name , concerned_person, department, id_2 as tin, taxable_amount, tot_vat, tot_sat, total_amount, tot_disc, other_discount, grand_total, timestamp, quantity, type, invoice_type, invoice_no,invoice_sale_id, agent_id, mode_of_payment, storeid from invoice_sale_restaurant_2 join customer on customer.sno = invoice_sale_restaurant_2.supplier_id where timestamp>="'.$_SESSION['sale_date_from'].'" and timestamp<="'.$_SESSION['sale_date_to'].'" and invoice_sale_restaurant_2.mode_of_payment !="nocharge" and edition_time !=""';
	//echo $sql;

	$sql_sum = 'select sum(taxable_amount) as taxable_amount, sum(tot_vat) as tot_vat, sum(tot_sat) as tot_sat, sum(total_amount) as total_amount, sum(grand_total) as grand_total, if(other_discount="", sum(tot_disc), sum(other_discount)) as total_discount, sum(quantity) as quantity from invoice_sale_restaurant_2 left join customer on customer.sno = invoice_sale_restaurant_2.supplier_id where timestamp>="'.$_SESSION['sale_date_from'].'" and timestamp<="'.$_SESSION['sale_date_to'].'" and invoice_sale_restaurant_2.mode_of_payment !="nocharge" and edition_time !=""';
	//echo $sql_sum;
	...
	$filter_summary='';
	$sql = 'select invoice_sale_restaurant_2.sno, cust_name,company_name, id_2 as tin, concerned_person, department, taxable_amount, tot_vat, tot_sat, total_amount, tot_disc, other_discount, grand_total, timestamp, quantity, type, invoice_type, invoice_no,invoice_sale_id, agent_id, mode_of_payment, storeid from invoice_sale_restaurant_2 left join customer on customer.sno = invoice_sale_restaurant_2.supplier_id where  invoice_sale_restaurant_2.mode_of_payment !="nocharge" and edition_time !="" order by timestamp desc, abs(invoice_sale_restaurant_2.invoice_no) desc';
	$result_data = execute_query($sql);	

	$sql_sum = 'select sum(taxable_amount) as taxable_amount, sum(tot_vat) as tot_vat, sum(tot_sat) as tot_sat, sum(total_amount) as total_amount, if(other_discount="", sum(tot_disc), sum(other_discount)) as total_discount, sum(grand_total) as grand_total, sum(quantity) as quantity from invoice_sale_restaurant_2 left join customer on customer.sno = invoice_sale_restaurant_2.supplier_id where edition_time !="" and invoice_sale_restaurant_2.mode_of_payment !="nocharge" ';
	$row_sum = mysqli_fetch_array(execute_query($sql_sum, dbconnect()));
        	...
        	<table width="100%">
            	...
            	<tr style="background:#CCC;">
                	...
                	<th>Mode of Payment</th>
                        ...
                        <option  value="all">All Invoices</option>
                        <?php
						$sql='select * from invoice_sale_restaurant_2 group by mode_of_payment';
...
?>
	<table width="100%">
    	...
    	<tr>
            ...
            <th>Kot No.</th>
            <th>Invoice No.</th>
            <th colspan="4" class="no-print">&nbsp;</th>
        </tr>
        </thead>
        <tbody>
        <?php
		...
		for ($pgid = $start; $pgid < $end; $pgid++) {
			...
			/**	if($row['concerned_person']!=''){
				...
				$qry="SELECT * FROM `invoice_sale_restaurant_2` WHERE invoice_no='".$row['invoice_no']."'";
				$res=execute_query($qry);
				$kotrow=mysqli_fetch_array($res);
				echo'<td>'.$kotrow['kot_no'].'</td>';
				echo '<td>'.$row['invoice_no'].'</td>
				
				<td class="no-print"><a href="scripts/printing_sale_restaurant_copy.php?inv='.$row['invoice_sale_id'].'" target="_blank">View</a></td>
				
				<td class="no-print"><a href="report_sale_modifiy.php?del='.$row['sno'].'" onclick="return confirm(\'Are you sure?\');"><img src="images/del.png" height="20"></a></td>
			</tr>';
    ...
    <table>
    	...
    	<?php
    	if(!isset($_POST['mop'])){
			$sql_summary = 'SELECT mode_of_payment, count(*) as count, sum(grand_total) as grand_total FROM `invoice_sale_restaurant_2` left join customer on customer.sno = invoice_sale_restaurant_2.supplier_id '.$filter_summary.' WHERE mode_of_payment !="nocharge"  and edition_time !="" group by mode_of_payment';
		}
		else{
			$sql_summary = 'SELECT mode_of_payment, count(*) as count, sum(grand_total) as grand_total FROM `invoice_sale_restaurant_2` left join customer on customer.sno = invoice_sale_restaurant_2.supplier_id '.$filter_summary.' and mode_of_payment !="nocharge" and edition_time !="" group by mode_of_payment';
			//echo $sql_summary;
		}
		//echo $sql_summary;
