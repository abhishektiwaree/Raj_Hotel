...
if(isset($_POST['submit'])){
	...
	$q="UPDATE `kitchen_ticket_temp` SET `table_id`='".$trow['sno']."' where table_id='".$_POST['table_id']."'";
	$res=execute_query($q);
	$sql="UPDATE `res_table` SET `booked_status`='0' where sno='".$_POST['table_id']."'";
	$res=execute_query($sql);
	$sql="UPDATE `res_table` SET `booked_status`='1' where sno='".$trow['sno']."'";
	$res=execute_query($sql);
	if($res){
		$msg="Table Changed";
		if($_POST['edit_sno_table'] != ''){
			$i="UPDATE `invoice_sale_restaurant` SET `storeid`='".$trow['sno']."' , `table_no`='".$trow['table_number']."' where `sno`='".$_POST['edit_sno_table']."'";
			execute_query($i);
			...
			execute_query($i_2);
			$s="UPDATE `stock_sale_restaurant` SET `table_id`='".$trow['sno']."' where `invoice_no`='".$_POST['edit_sno_table']."'";
			execute_query($s);
			$s_2="UPDATE `stock_sale_restaurant_2` SET `table_id`='".$trow['sno']."' where `invoice_no`='".$_POST['edit_sno_table']."'";
			execute_query($s_2);
			header("Location: dine_in_order.php?edit_id=".$_POST['edit_sno_table']."");
		}
		else{
...
if(isset($_POST['save'])){
	...
	$q="UPDATE `kitchen_ticket_temp` SET `table_id`='".$newtab."' where table_id='".$oldtab."'";
	$res=execute_query($q);
	$sql="UPDATE `room_master` SET `booked_status`='0' where sno='".$_POST['room_id']."'";
	$res=execute_query($sql);
	$sql="UPDATE `room_master` SET `booked_status`='1' where sno='".$rrow['sno']."'";
	$res=execute_query($sql);
	if($res){
		$msg="Room Changed";
		if($_POST['edit_sno_room'] != ''){
			$i="UPDATE `invoice_sale_restaurant` SET `storeid`='".$newtab."' where `sno`='".$_POST['edit_sno_room']."'";
			execute_query($i);
			$i_2="UPDATE `invoice_sale_restaurant_2` SET `storeid`='".$newtab."' where `invoice_sale_id`='".$_POST['edit_sno_room']."'";
			execute_query($i_2);
			$s="UPDATE `stock_sale_restaurant` SET `table_id`='".$newtab."' where `invoice_no`='".$_POST['edit_sno_room']."'";
			execute_query($s);
			$s_2="UPDATE `stock_sale_restaurant_2` SET `table_id`='".$newtab."' where `invoice_no`='".$_POST['edit_sno_room']."'";
			execute_query($s_2);
			header("Location: dine_in_order.php?edit_id=".$_POST['edit_sno_room']."");
		}
		else{
